  ...
  handler: async (request, context) => {
    try {
      ...
      const eventResult = await pool
        ...
        .input('label', sql.NVarChar, `${playerName} - ${statLabel}`).query(`
      ...
      const marketResult = await pool
        ...
        .input('type', sql.VarChar, 'player_prop').query(`
      ...
      const selectionResult = await pool
        ...
        .input(
          'label',
          sql.NVarChar,
          `${direction === 'more' ? '↑ More' : '↓ Less'} ${statValue} ${statLabel}`,
        )
        .input('odds', sql.Decimal(10, 4), odds)
        .input('lineValue', sql.Decimal(10, 2), statValue).query(`
